...
47 module đã có đủ mapping/spec/engine. Course 01–03 (108 bài) vẫn dùng các engine chuyên biệt. Mỗi trong 200 bài Course 04–13 có một blueprint theo đúng `sourcePath`, với activity ID, scenario, thao tác chính, tiêu chí thành công, workflow, checkpoint và challenge riêng. `DedicatedLessonLab` tái sử dụng khung UI nhưng chọn visual/control theo 46 simulation family (tensor, autograd, regression, optimization, Transformers, fine-tuning, RAG, agent, UI flow, v.v.), không còn dùng profile chung theo module. Kiểm thử tự động phải import/validate đủ 308 spec, kiểm tra 200 blueprint và mount đại diện của cả 46 family. Chưa kiểm thử ghi tiến độ vào Firebase để tránh thay đổi dữ liệu thật. Con số 47/47 là **coverage simulation trong catalog**, không phải tiến độ học của tài khoản hay bằng chứng QA thủ công cho từng bài.
...
| `assets/js/simulations/lessons/*.js` | 108 spec chuyên biệt; `dedicated-course-blueprints.js` chứa 200 blueprint theo `sourcePath`, và `dedicated-course-spec.js` tạo spec tương ứng. |
| `assets/js/simulations/engines/*.js` | Logic tương tác; mỗi engine cần `mount`, `update`, `reset`, `getAccessibleSummary`, `destroy`. `DedicatedLessonLab` render activity theo family/blueprint, không theo module. |
...
| `tests/test_simulation_integration.py`, `tests/simulation-contract.test.mjs` | Kiểm tra mapping, spec, engine và host integration; Node suite cũng xác minh đủ 200 blueprint/46 family và mount renderer đại diện từng family. |
...
Mục tiêu cuối: **308/308 bài `ready`, 47/47 module phủ hết**; snapshot hiện tại đạt **308/308 bài, 47/47 module**, 46 engine. Course 04–13 đã chuyển từ profile chung theo module sang 200 blueprint riêng và 46 family; Python **8/8** kiểm tra manifest/loader, Node **30/30** kiểm tra factory/spec/engine cho 308 bài và mount đại diện cả 46 family. Chrome smoke-test đã xác nhận các bài Tensor, RAG và Gradio/QA Bot hiển thị visual/workflow/control/challenge đúng blueprint riêng; target preset và Reset hoạt động, không ghi tiến độ Firebase trong QA.
